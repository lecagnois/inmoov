...
def mailsend(amis):
	try:
		...
		dico = {"jean-pierre":"toto@wanadoo.fr","tata":"tata@wanadoo.fr"}
...
def photo(msg):
	...
	talkBlocking("la photo est enregistrer , DIRE OUI PHOTO pour envoyer un mel au concepteur")
	sleep(2)
...
def photoui():
   ...
   #fromaddr = "toto@gmail.com"
   fromaddr = "toto@wanadoo.fr"
   toaddr = "toto@wanadoo.fr"
   msg = MIMEMultipart()
   msg['From'] = fromaddr
   msg['To'] = toaddr
   msg['Subject'] = "photographie"
...
def liremail(destinataire):
	try :
		...
		# creation d un dictionnaire des amis
		dico = {"jean-pierre":'(FROM "toto")',"colette":'(FROM "tata")', "robot":'(FROM "inmoov")'}
		# message si pas trouver
		if (dico.get(destinataire) == None):
			print("Amis inconnu dans l annuaire")
			talkBlocking("personne inconnu au bataillon")
			...
			mail.login('toto@wanadoo.fr', 'votre mot de passe')
			mail.list() 
			...
			#result, data = mail.uid('search', None, '(FROM "toto")')
